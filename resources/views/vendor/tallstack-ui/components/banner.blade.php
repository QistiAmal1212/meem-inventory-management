...
@if ($show)
    <div  x-cloak x-data="tallstackui_banner(@js($flash), @js($animated), @js($wire), @js($text ??= $slot->toHtml()), @js($enter), @js($leave), @js($close))"
        @class([
        ...
        @if ($wire)
            <div @class([$personalize['text'], 'flex justify-center items-center gap-2'])>
                ...
                <div x-cloak x-show="type === 'warning'">
                    <x-dynamic-component :component="TallStackUi::prefix('icon')"
